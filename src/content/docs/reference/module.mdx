...
A module is a self-contained part of the build executed during the build process of an image. Most modules are bash scripts ([blue-build/modules](https://github.com/ublue-os/bling)), but some default modules are implemented with Containerfile templating directly in [blue-build/cli](https://github.com/blue-build/cli). Modules are configured in the [recipe](/reference/recipe/) or an external configuration file.
