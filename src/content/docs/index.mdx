...
hero:
  ...
  actions:
    - text: Let's get started!
      ...
      icon: "<svg material-symbols:book-5-rounded"
      variant: primary
    - text: GitHub
...
<hr class="opacity-0"/>
<CardGrid stagger>
	<Card title="Easy configuration" icon="material-symbols:edit-outline-rounded">
		The build process can be configured entirely in a straightforward YAML format called a [recipe](/reference/recipe/).
    When building, the recipe is transformed into a Containerfile by the BlueBuild cli tool.
	</Card>
	<Card title="Totally extensible" icon="material-symbols:extension-rounded">
		The modular build system [can easily be extended](/how-to/making-modules/) by just writing a shell script.
	...
	<Card title="Based on well-known standards" icon="material-symbols:book-2-rounded">
		The format of your OS images is the familiar OCI image format, famously used by Docker & Podman.
    With [Fedora's Native Container support](https://fedoraproject.org/wiki/Changes/OstreeNativeContainerStable), it becomes possible to boot natively into a OCI compatible image.
	</Card>
	<Card title="Free & open-source" icon="ri:open-source-fill">
		BlueBuild is an open-source project built by the community for the community.
    Feel free to peek at & tinker with the [source code](https://github.com/blue-build/)!
    GitHub also provides CI/CD & a container registry for free, though other providers are supported as well.
	</Card>
  <Card title="Effortless reliability" icon="ph:scales-bold">
  ...
  <Card title="Build on the best" icon="material-symbols:stacks-rounded">
    Build your custom images on [Universal Blue](https://universal-blue.org/)'s awesome base, with the option to build on [atomic Fedora desktops](https://fedoraproject.org/atomic-desktops/) and more options being supported soon.
  </Card>
</CardGrid>
...
If you're just looking to build a simple container image to run on the cloud or locally, a Containerfile might still be the easiest way to get the result you want. If you need some help setting up the GitHub Actions for that, check out our [guide](/how-to/from-scratch/) for building a build system from scratch.
