...
:::caution
These commands will be replaced by `switch` in `v0.9.0`. It will be aliased with `rebase` and `upgrade` so as to not break existing scripts. Right now, `upgrade` must be called when building the same image you called `rebase` on earlier. The `switch` command removes this ambiguity.
:::
