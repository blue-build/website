...
4. Fix merge conflicts in a merge editor. At least Visual Studio Code supports this feature by default, and other tools might do too, so use what you're most comfortable with.
    ...
    - You can totally ignore changes in the `config/` and `modules/` directories, the `README.md` and the `cosign.pub`, so that your customizations are not overwritten. Otherwise use your own discretion.
