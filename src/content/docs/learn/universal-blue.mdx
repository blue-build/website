...
1.  Generate the ISO:
    ...
    -   With `podman`:
        ...
        # iso command:
        mkdir ./"$IMAGE_NAME"_iso
        sudo podman run --rm --privileged --volume ./"$IMAGE_NAME"_iso:/build-container-installer/build --security-opt label=disable --pull=newer \
    ...
    -   With `docker`:
        ...
        # iso command:
        mkdir ./"$IMAGE_NAME"_iso
        sudo docker run --rm --privileged --volume ./"$IMAGE_NAME"_iso:/build-container-installer/build \
        -e IMAGE_REPO="$IMAGE_REPO" -e IMAGE_NAME="$IMAGE_NAME" -e IMAGE_TAG="$IMAGE_TAG" -e VARIANT="$IMAGE_VARIANT" \
        ghcr.io/jasonn3/build-container-installer:latest
        ```
...
2.  Flash the ISO onto a USB drive ([Fedora Media Writer](https://www.fedoraproject.org/en/workstation/download) is recommended) and boot it, or boot it directly with a virtual machine.
        ...
        - The ISO file should be in a directory named after your `IMAGE_NAME` inside your working directory and called something like `deploy.iso`.
