...
You can use the `bluebuild generate-iso` to generate an offline ISO of your image locally with BlueBuild CLI. The command runs JasonN3's [build-container-installer](https://github.com/JasonN3/build-container-installer) under the hood. See the upstream project's README for more information and instructions for advanced use cases.

1.  Generate the ISO:

    ```bash
    # Generate ISO from a built and published image
    bluebuild generate-iso weird-os image ghcr.io/octocat/weird-os

    # Build image and generate ISO from a local recipe
    bluebuild generate-iso weird-os recipe ./recipes/recipe.yml
    ```
