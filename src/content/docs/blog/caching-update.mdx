...
description: A recent BlueBuild CLI update makes builds and updates much faster.
date: 2024-04-27
authors:
...
---
:::tip[TLDR]
A recent BlueBuild CLI update makes builds and updates much faster. [What do I need to do to get it?](#migration)
:::
...
I'm happy to announce today that we have now released `v0.8.4` of `bluebuild` CLI which completes the last set of changes needed to allow users to quickly iterate on changes to their build to try new changes without having to wait for long build times or large downloads. The change is as simple as:
